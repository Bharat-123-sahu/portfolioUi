...
export class SkillsService extends BaseCrudService<Skill>{
  ...
  protected override endpoint =
      `${environment.apiUrl}/api/v1/admin/skills`;



 getFeatured(): Observable<Skill[]> {
    return this.http.get<Skill[]>(`${environment.apiUrl}api/v1/admin/skills/featured`);
  }
  ...
  getByCategory(category: string): Observable<Skill[]> {
    return this.http.get<Skill[]>(
      `${environment.apiUrl}api/v1/admin//skills/category/${category}`
    );
  }
